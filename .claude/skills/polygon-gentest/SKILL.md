...
Emit the test with `cout` only. Do not use `printf`, `puts`, `putchar`, or any other C stdio call in the generator; testlib's own helpers (`println`, `rnd`) remain fine. Separate tokens with `' '` and end lines with `'\n'`.
