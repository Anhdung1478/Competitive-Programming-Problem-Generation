...
- I/O style: this repository prefers `cin`/`cout` over `scanf`/`printf`. `source/solution.cpp` is a source-of-truth file, so do not rewrite its I/O for this reason — report the deviation and keep the audit going. Every file the agent itself writes (brute forces, scratch programs, `outputs/solution/*.cpp`) must use `cin`/`cout` with `ios_base::sync_with_stdio(false); cin.tie(nullptr);`;
