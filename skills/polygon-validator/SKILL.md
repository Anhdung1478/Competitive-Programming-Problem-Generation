...
Use `inf.readInt`, `inf.readLong`, `inf.readToken`, or another typed `testlib` reader with explicit bounds and variable names. Use `readSpace`, `readEoln`, and `readEof` to enforce the declared layout. Do not use `cin`, `scanf`, `printf`, or any other C stdio call, and no random generation, jury answers, or participant output. If a plain stream write is ever needed, use `cout`.
...
1. run the validator on the statement's sample inputs and on any inputs already used during Step 4 validation; once `outputs/gentest.cpp` exists, run it on representative outputs from every `test-script.txt` profile as well;
2. cover minimum, maximum, each subtask, and structural edge cases;
3. confirm every sampled input that should be legal is accepted;
